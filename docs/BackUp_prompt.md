 ...
 *
 * v3 — Missing first row fix (2026-02-13)
 *      Added explicit instructions to NOT skip first row even with high values.
 *      Added position number verification and sum-to-subtotal self-check.
 *
 * v2 — NET/GROSS semantic clarity fix (2026-02-13)
 *      Explicitly specifies that lineItems.totalPrice MUST be NET (before VAT).
 *      Prevents BR-CO-10 and BR-CO-14-SUM validation failures caused by
 *      AI extracting GROSS line totals from invoices.
 */
...

{
  ...
  "sellerContactName": "string or null — contact person name, or copy sellerName if no separate contact shown",
  ...
  "dueDate": "string (YYYY-MM-DD) or null — payment due date if shown (look for 'Zahlbar bis', 'Fällig am', 'Due date')",

  "lineItems": [
    {
      "positionNumber": "number or null",
      "description": "string",
      "quantity": "number",
      "unitPrice": "number — unit price per item (e.g. price per day, per piece, per km). This is the NET unit price (before VAT).",
