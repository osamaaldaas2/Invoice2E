...
function getSessionSecret(): string {
  ...
  if (!_devSecret) {
    ...
    logger.warn(
      'SESSION_SECRET not set — using random per-process secret. Sessions will not persist across restarts.'
    );
  }
  return _devSecret;
}
...
export function verifySessionToken(token: string): SessionPayload | null {
  try {
    ...
    // FIX: Re-audit #2 — validate aud claim to prevent cross-deployment token reuse
    if (payload.aud) {
      const expectedAudience = process.env.NEXT_PUBLIC_APP_URL || 'https://invoice2e.com';
...
export async function getSessionWithProfile(): Promise<
  (SessionPayload & { email: string; firstName: string; lastName: string }) | null
> {
