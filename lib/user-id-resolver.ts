...
export async function resolveUserIds(user: AuthenticatedUser): Promise<ResolvedUserIds> {
  // User-scoped client for public.users queries (RLS-protected)
  const userClient = await createUserScopedClient(user.id);
  // Admin client for auth.users schema access (requires service role)
  ...
  const { data: publicIdData, error: publicIdError } = await userClient
    .from('users')
    .select('id')
    .eq('id', user.id)
    .maybeSingle();
  ...
  if (normalizedEmail) {
    const { data: publicEmailData, error: publicEmailError } = await userClient
      .from('users')
      .select('id')
  ...
  try {
    const { data: authIdData } = await adminClient
      .schema('auth')
      .from('users')
      .select('id')
      .eq('id', user.id)
  ...
  if (normalizedEmail) {
    try {
      const { data: authEmailData } = await adminClient
        .schema('auth')
        .from('users')
        .select('id')
        .eq('email', normalizedEmail)
