...
export const EmailSchema = z.string().email('Invalid email address').max(INPUT_LIMITS.EMAIL_MAX, 'Email too long');
...
export const SignupSchema = z.object({
    ...
    firstName: z.string().min(2, 'First name too short').max(INPUT_LIMITS.NAME_MAX, 'First name too long'),
    lastName: z.string().min(2, 'Last name too short').max(INPUT_LIMITS.NAME_MAX, 'Last name too long'),
    addressLine1: z.string().min(2, 'Address is required').max(INPUT_LIMITS.ADDRESS_MAX, 'Address too long'),
    ...
    postalCode: z.string().min(3, 'Postal code is required').max(INPUT_LIMITS.POSTAL_CODE_MAX, 'Postal code too long'),
...
export const LoginSchema = z.object({
    email: EmailSchema,
    password: z.string().min(1, 'Password required').max(INPUT_LIMITS.PASSWORD_MAX, 'Password too long'),
});
...
export const UpdateProfileSchema = z.object({
    ...
    phone: z.string().max(INPUT_LIMITS.PHONE_MAX).regex(/^\+?[\d\s\-().]*$/, 'Invalid phone format').optional(),
