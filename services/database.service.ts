...
// FIX: Re-audit #4 — replace dangerous CreditService with safe CreditsDatabaseService
...
export class DatabaseService {
  ...
  // ==================== CREDITS OPERATIONS ====================
  // FIX: Re-audit #4 — delegate to safe CreditsDatabaseService (atomic RPCs, input validation)

  async getUserCredits(userId: string): Promise<UserCredits> {
    return creditsDbService.getUserCredits(userId);
  }

  async createUserCredits(userId: string, initialCredits: number = 0): Promise<UserCredits> {
    return creditsDbService.createCredits(userId, initialCredits);
  }

  async deductCredits(userId: string, amount: number = 1): Promise<boolean> {
    return creditsDbService.deductCredits(userId, amount);
  }

  async addCredits(userId: string, amount: number): Promise<UserCredits> {
    return creditsDbService.addCredits(userId, amount);
  }
