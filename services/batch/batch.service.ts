...
export class BatchService {
  ...
  private async ensureInputBucket(): Promise<void> {
    ...
    const exists = (buckets || []).some(
      (bucket: { name: string }) => bucket.name === this.INPUT_BUCKET
    );
    if (!exists) {
  ...
  private async parseZip(zipBuffer: Buffer): Promise<ParsedZip> {
    if (zipBuffer.length > this.MAX_TOTAL_SIZE) {
      throw new ValidationError(
        `ZIP file too large. Maximum size is ${this.MAX_TOTAL_SIZE / 1024 / 1024}MB`
      );
    }
  ...
  private async uploadInputZip(path: string, zipBuffer: Buffer): Promise<void> {
    ...
    const { error } = await supabase.storage.from(this.INPUT_BUCKET).upload(path, zipBuffer, {
  ...
  private async downloadInputZip(path: string): Promise<Buffer> {
    ...
    const { data, error } = await supabase.storage.from(this.INPUT_BUCKET).download(path);

    if (error || !data) {
      throw new AppError(
        'STORAGE_ERROR',
        `Failed to download input ZIP: ${error?.message || 'Missing file'}`,
        500
      );
    }
  ...
  async createBatchJob(userId: string, zipBuffer: Buffer): Promise<BatchJob> {
    ...
    if (error || !job) {
      ...
      throw new AppError(
        'DATABASE_ERROR',
        `Failed to create batch job: ${error?.message || 'unknown error'}`,
        500
      );
    }
    ...
    if (pathError) {
      logger.error('Failed to update batch input file path', {
        jobId: job.id,
        error: pathError.message,
      });
      throw new AppError(
        'DATABASE_ERROR',
        `Failed to update batch input path: ${pathError.message}`,
        500
      );
    }
  ...
  async createMultiInvoiceJob(
    ...
    if (error || !job) {
      throw new AppError(
        'DATABASE_ERROR',
        `Failed to create multi-invoice job: ${error?.message || 'unknown'}`,
        500
      );
    }
    ...
    await supabase.from('batch_jobs').update({ input_file_path: inputPath }).eq('id', job.id);
  ...
  async processMultiInvoiceJob(jobId: string): Promise<void> {
    ...
    const boundaryData = job.boundary_data as {
      invoices: { pages: number[]; label: string }[];
    } | null;
  ...
  private async downloadInputFile(path: string): Promise<Buffer> {
    ...
    if (error || !data) {
      throw new AppError(
        'STORAGE_ERROR',
        `Failed to download input file: ${error?.message || 'Missing file'}`,
        500
      );
    }
  ...
  async recoverStuckJobs(thresholdMs: number = 300000): Promise<number> {
    ...
    for (const job of stuckJobs) {
      ...
      const completedFiles = job.completed_files ?? 0;
      const failedFiles = job.failed_files ?? 0;
      const totalFiles = job.total_files ?? 0;
      const allProcessed = totalFiles > 0 && completedFiles + failedFiles >= totalFiles;
      ...
      if (allProcessed) {
        ...
        await supabase
          ...
          .eq('id', job.id)
          .eq('status', 'processing');
        logger.info('Stuck batch job finalized (all files already processed)', {
          jobId: job.id,
          finalStatus,
          completedFiles,
          failedFiles,
  ...
  async runWorkerOnce(): Promise<boolean> {
    ...
    if (pendingError) {
      ...
      throw new AppError(
        'DATABASE_ERROR',
        `Failed to query pending jobs: ${pendingError.message}`,
        500
      );
    }
    ...
    if (claimError) {
      logger.warn('Failed to claim pending batch job', {
        jobId: pending.id,
  ...
  async getBatchStatus(userId: string, jobId: string): Promise<BatchProgress | null> {
    ...
    const progress = job.total_files > 0 ? Math.round((processed / job.total_files) * 100) : 0;
  ...
  async cancelBatchJob(userId: string, jobId: string): Promise<boolean> {
    ...
    if (error) {
      logger.warn('Failed to cancel batch job', { jobId, userId, error: error.message });
      return false;
    }
