    ...
    if ('extractWithText' in this.adapter && this.adapter.extractWithText) {
      ...
      if (ENABLE_TEXT_EXTRACTION) {
        try {
          ...
          if (textResult.hasText) {
            ...
            logger.info('Pre-extracted text available for DeepSeek', {
      ...
      if (
        ...
        while (!validation.valid && shouldRetry(attempt)) {
          attempt++;
          logger.info('DeepSeek extraction validation failed, retrying', {
            attempt,
        ...
        if (!validation.valid) {
          ...
          logger.warn('DeepSeek extraction still has validation errors after retries', {
