...
export class PaymentProcessor {
  ...
  async processPayment(
    ...
    if (method === 'stripe') {
      ...
      const order = await paypalService.createOrder(userId, packageId, successUrl, cancelUrl);
  ...
  async handleStripeWebhook(event: {
    ...
    if (sessionId) {
      ...
      if (verifyEvent) {
        logger.info(
          'Payment already processed by verify endpoint, skipping webhook credit addition',
          {
            eventId,
            sessionId,
            userId,
  ...
  async handlePaypalWebhook(event: {
    ...
    try {
      const purchaseUnits = event.resource.purchase_units as
