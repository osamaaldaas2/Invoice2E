...
export class UBLService {
  ...
  private readonly customizationId =
  ...
  async generate(data: UBLInvoiceData): Promise<string> {
    ...
    try {
      ...
      throw new Error(
        `UBL generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
...
<CreditNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2"
  ...
  private buildCreditNoteLines(data: UBLInvoiceData): string {
    return data.lineItems
      .map((item, index) => this.buildCreditNoteLine(item, index + 1, data.currency))
      .join('\n');
  }
  ...
  private buildAccountingSupplierParty(data: UBLInvoiceData): string {
    ...
    const vatId =
      data.sellerVatId || (data.sellerTaxId && isEuVatId(data.sellerTaxId) ? data.sellerTaxId : '');
  ...
  /**
   * Compute tax breakdown from line items and allowances/charges.
   * Returns per-group details and total tax amount.
   *
   * BR-CO-14 reconciliation: when the invoice's stored taxAmount is within
   * rounding tolerance (≤ 0.02) of the computed total, we prefer the stored
   * value and adjust the last group's taxAmount so Σ subtotals == TaxTotal.
   * This mirrors the approach used by the CII builder.
   */
  private computeTaxBreakdown(data: UBLInvoiceData): {
    groups: Array<{ rate: number; taxableAmount: number; categoryCode: string; taxAmount: number }>;
    totalTax: number;
  } {
    ...
    const groups = Array.from(taxGroups.entries())
      .sort((a, b) => b[0] - a[0])
      .map(([rate, group]) => ({
        rate,
    ...
    let totalTax = sumMoney(groups.map((g) => g.taxAmount));

    const storedTax = roundMoney(data.taxAmount);
    const diff = roundMoney(storedTax - totalTax);
    if (storedTax > 0 && Math.abs(diff) > 0 && Math.abs(diff) <= 0.02 && groups.length > 0) {
      groups[groups.length - 1]!.taxAmount = roundMoney(
        groups[groups.length - 1]!.taxAmount + diff
      );
      totalTax = storedTax;
    }

    return { groups, totalTax };
  }

  /**
   * Build TaxTotal with multi-rate support.
   * Uses computeTaxBreakdown to ensure BR-CO-14: TaxTotal == Σ TaxSubtotals.
   */
  private buildTaxTotal(data: UBLInvoiceData): string {
    const { groups, totalTax } = this.computeTaxBreakdown(data);
      ...
      .map(
        ...
        <cac:TaxSubtotal>
            ...
            <cac:TaxCategory>
                ...
                <cbc:Percent>${group.rate.toFixed(2)}</cbc:Percent>${this.buildTaxExemptionReason(group.categoryCode)}
                <cac:TaxScheme>
                    <cbc:ID>VAT</cbc:ID>
                </cac:TaxScheme>
            </cac:TaxCategory>
    ...
    return `
    <cac:TaxTotal>
        <cbc:TaxAmount currencyID="${data.currency}">${totalTax.toFixed(2)}</cbc:TaxAmount>${subtotals}
    </cac:TaxTotal>`;
  }
   ...
   * Derives TaxInclusiveAmount and PayableAmount from computed values
   * to ensure BR-CO-13/BR-CO-15 consistency.
   */
  private buildLegalMonetaryTotal(data: UBLInvoiceData): string {
    ...
    const taxExclusive = roundMoney(lineExtension - totalAllowances + totalCharges);

    // BR-CO-15: TaxInclusiveAmount = TaxExclusiveAmount + TaxTotal (from computed breakdown)
    const { totalTax } = this.computeTaxBreakdown(data);
    const taxInclusive = roundMoney(taxExclusive + totalTax);
    ...
    <cac:LegalMonetaryTotal>
        ...
        <cbc:TaxExclusiveAmount currencyID="${data.currency}">${taxExclusive.toFixed(2)}</cbc:TaxExclusiveAmount>
        <cbc:TaxInclusiveAmount currencyID="${data.currency}">${taxInclusive.toFixed(2)}</cbc:TaxInclusiveAmount>
  ...
  private buildInvoiceLines(data: UBLInvoiceData): string {
    return data.lineItems
      .map((item, index) => this.buildInvoiceLine(item, index + 1, data.currency))
      .join('\n');
  }
  ...
  private formatDate(date: string): string {
    ...
    if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(date)) {
      ...
      throw new Error(
        `Ambiguous date format "${date}". Use ISO (YYYY-MM-DD) or German (DD.MM.YYYY)`
      );
    }
  ...
  async validate(xml: string): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (!xml.includes('xeinkauf.de:kosit:xrechnung_3.0')) {
      errors.push(
        'Missing XRechnung 3.0 customization ID (must use xeinkauf.de:kosit:xrechnung_3.0)'
      );
    }
