...
import { IStripeAdapter, StripeCheckoutParams, StripeEvent, StripeRefund, StripeSession } from './interfaces';
...
export class StripeAdapter implements IStripeAdapter {
    ...
    async createCheckoutSession(params: StripeCheckoutParams): Promise<StripeSession> {
        ...
        if (!params.metadata?.credits) bodyParams.append('metadata[credits]', params.credits.toString());
