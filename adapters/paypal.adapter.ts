...
export class PayPalAdapter implements IPayPalAdapter {
  ...
  private validateOrderId(orderId: string): void {
    if (!/^[a-zA-Z0-9_]+$/.test(orderId) || orderId.length > 255) {
  ...
  async captureOrder(orderId: string): Promise<PayPalCapture> {
    ...
    const response = await fetch(
      `${this.apiUrl}/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  async getOrder(orderId: string): Promise<PayPalOrder> {
    ...
    const response = await fetch(
      `${this.apiUrl}/v2/checkout/orders/${encodeURIComponent(orderId)}`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
