...
export default function ConvertPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      // Get the review data from sessionStorage, with DB fallback (F-07)
      let reviewData = sessionStorage.getItem(`review_${extractionId}`);

      if (!reviewData) {
        // Fallback: fetch reviewed extraction data from DB
        const res = await fetch(`/api/invoices/extractions/${extractionId}`);
        if (!res.ok) {
          throw new Error(t('noReviewData'));
        }
        const dbData = await res.json();
        const extractionData = dbData.data?.extractionData;
        if (!extractionData) {
          throw new Error(t('noReviewData'));
        }
        // Use DB data as review payload (outputFormat may come from conversion record)
        reviewData = JSON.stringify(extractionData);
      }
      ...
      if (Array.isArray(data.validationWarnings) && data.validationWarnings.length > 0) {
        ...
        Array.isArray(data.data?.validationWarnings) &&
        data.data.validationWarnings.length > 0
      ) {
              ...
              <div className="flex items-center justify-between gap-2 sm:gap-4">
                ...
                <div className="flex-1 flex items-center justify-center gap-2 sm:gap-4 min-w-0">
                  <div className="flex items-center gap-1 sm:gap-2">
                    ...
                    <span className="text-emerald-200 font-medium text-xs sm:text-sm hidden sm:inline">
                      {t('stepUpload')}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1 sm:gap-2">
                    ...
                    <span className="text-sky-200 font-medium text-xs sm:text-sm hidden sm:inline">
                      {t('stepConvert')}
                    </span>
                  </div>
                </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <ul className="space-y-2">
                            {validationErrors.map((ve, idx) => (
                              <li
                                key={idx}
                                className="p-3 bg-rose-500/10 border border-rose-400/20 rounded-lg"
                              >
                        ...
                        <div className="p-4 glass-panel border border-white/10 rounded-lg">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-xs text-faded">
                              invoice_{outputFormat}
                              {getFormatMetadata(outputFormat).fileExtension}
                            </span>
                          </div>
                      ...
                      {pdfBlob && (
                        <div className="p-4 glass-panel border border-white/10 rounded-lg text-center">
                          <p className="text-sm text-slate-300 mb-2">
                            📄 PDF invoice generated ({(pdfBlob.size / 1024).toFixed(1)} KB)
                          </p>
                        </div>
                      )}
