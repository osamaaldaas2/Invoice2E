...
export async function POST(req: NextRequest) {
  try {
      ...
      try {
        ...
        if (session.payment_status === 'paid' && session.metadata?.credits) {
          ...
          stripePaymentIntentId =
            session.paymentIntentId || (session as { payment_intent?: string }).payment_intent;
        }
      } catch (error) {
    ...
    if (orderId && !verified) {
      try {
        let order = await paypalAdapter.getOrder(orderId);
        ...
        if (order.status === 'APPROVED') {
          ...
          order = await paypalAdapter.getOrder(orderId);
        ...
        if (order.status === 'COMPLETED' && order.customId) {
          ...
          logger.warn('PayPal order not in COMPLETED status', { orderId, status: order.status });
        }
      } catch (error) {
    ...
    if (verified && credits > 0) {
      ...
      const { data: transaction } = await supabase
        ...
        .eq('user_id', user.id)
        .or(
          `stripe_session_id.eq.${paymentId},stripe_payment_id.eq.${paymentId},paypal_order_id.eq.${paymentId}`
        )
