...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        const [
            ...
            supabase.from('users').select('id, email, first_name, last_name, role, created_at').eq('id', user.id).single(),
            supabase.from('user_credits').select('*').eq('user_id', user.id).single(),
            supabase.from('invoice_extractions').select('id, file_name, status, created_at').eq('user_id', user.id),
            supabase.from('invoice_conversions').select('id, conversion_format, status, created_at').eq('user_id', user.id),
            supabase.from('payment_transactions').select('id, amount, currency, status, created_at').eq('user_id', user.id),
        ]);
