...
export default function CreditHistory() {
  ...
  const getSourceDisplay = useCallback(
    (source: string): { label: string; icon: string } => {
  ...
  return (
    <div className="glass-card p-4 sm:p-6">
      <h3 className="text-lg font-semibold text-white font-display mb-4">{t('historyTitle')}</h3>
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-faded">{formatDate(tx.created_at)}</p>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p
                      className={`font-semibold ${isCredit ? 'text-emerald-300' : 'text-rose-300'}`}
                    >
                      {isCredit ? '+' : '-'}
                      {absAmount} {t('credit', { count: absAmount })}
                    </p>
