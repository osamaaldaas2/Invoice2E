...
describe('ReviewService', () => {
  ...
  const validData: ReviewedInvoiceData = {
    ...
    lineItems: [{ description: 'Product A', quantity: 2, unitPrice: 50, totalPrice: 100 }],
    subtotal: 100,
    taxAmount: 19,
    totalAmount: 119,
    currency: 'EUR',
  ...
  describe('validateReviewedData', () => {
    ...
    it('should accept data with optional email fields empty when electronic address provided', () => {
      const data = {
        ...validData,
        buyerEmail: '',
        sellerEmail: '',
        buyerElectronicAddress: 'buyer@peppol.de',
        sellerElectronicAddress: 'seller@peppol.de',
        paymentTerms: '',
        notes: '',
      };
    ...
    it('should reject when both buyerEmail and buyerElectronicAddress are empty', () => {
      const data = {
        ...validData,
        buyerEmail: '',
        buyerElectronicAddress: '',
      };
      expect(() => service.validateReviewedData(data)).toThrow(
        'Buyer electronic address is required'
      );
    });

    it('should reject when both sellerEmail and sellerElectronicAddress are empty', () => {
      const data = {
        ...validData,
        sellerEmail: '',
      ...
      expect(() => service.validateReviewedData(data)).toThrow(
        'Seller electronic address is required'
      );
    });

    it('should accept non-email endpoint as buyerElectronicAddress', () => {
      const data = {
        ...validData,
        buyerEmail: '',
        buyerElectronicAddress: '0204:123456789',
      };
      expect(service.validateReviewedData(data)).toBe(true);
    });
  });
  ...
  describe('recalculateTotals', () => {
    ...
    it('should handle items without tax rate', () => {
      const lineItems = [{ description: 'A', quantity: 1, unitPrice: 100, totalPrice: 100 }];
      const result = service.recalculateTotals(lineItems);
  ...
  describe('T4: normalizeForPersistence', () => {
    it('should convert empty strings to null', () => {
      const data = {
        ...
        sellerEmail: '',
        notes: '   ',
        buyerElectronicAddress: 'buyer@test.com',
      };
      const result = service.normalizeForPersistence(data);
      expect(result.buyerEmail).toBeNull();
      expect(result.sellerEmail).toBeNull();
      expect(result.notes).toBeNull();
      expect(result.buyerElectronicAddress).toBe('buyer@test.com');
    });

    it('should preserve non-empty string values', () => {
      const result = service.normalizeForPersistence(validData);
      expect(result.invoiceNumber).toBe('INV-001');
      expect(result.buyerName).toBe('Test Buyer GmbH');
      expect(result.sellerName).toBe('Test Seller AG');
    });
